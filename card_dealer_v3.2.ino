...
void motor_delay(float delay_time){
  for(int i=0;i<((delay_time/1000)*80000);i++)myservo.write(90);
}
...
void servo(int deg1){
  while(true){
    ...
    bno.getEvent(&event);
    delay(5);
    int deg2=event.orientation.x-deg1;
    Serial.println(deg2);
     if(abs(deg2)<=0){
      myservo.write(90);
...
void choose5(){//功能選擇(繼續,退出)
  while(button_val1==0){
    if(cny70<analogRead(cny70<cny70_val)){
      button_scan(2);
    }
    else button_scan(1);//選擇功能
    button_scan(1);
    if(button_flag==1){
      lcd.clear();
      lcd.setCursor(0,0);
      ...
      if(analogRead(cny70<cny70_val)){
        lcd.clear();
        lcd.setCursor(0,0);
        lcd.print("inserted");
      }
      if(button_val==1&&analogRead(cny70<cny70_val)){
        Serial.println("2 : continue");
        lcd.setCursor(0,1);
        lcd.print("2 : continue");
      }
...
void dealing_poker(){//撲克牌發牌
  ...
  lcd.setCursor(0,0);
  lcd.print("insert card");
  int a=millis(),b=millis();
  while(true){
     if(analogRead(cny70)<cny70_val)b=millis();
     else a=millis();
     if(analogRead(cny70)<cny70_val&&b-a>2000)break;
     delay(5);
  }
  lcd.clear();
  lcd.setCursor(0,0);
